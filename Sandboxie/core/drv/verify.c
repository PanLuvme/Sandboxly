...
// Forced to active/eCertEternal/max-level certificate with all options enabled
SCertInfo Verify_CertInfo = { 0xF000E401ULL };

_FX NTSTATUS KphValidateCertificate()
{
    // Certificate validation bypassed - Verify_CertInfo is hardcoded to active/max-level
    return STATUS_SUCCESS;
}
